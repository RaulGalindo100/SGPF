...
public class SubProcesoJpaController implements Serializable {
    ...
    public List<SubProceso> findSPByIndiceActividadMayor(int idPF, int indice){
    	EntityManager em = getEntityManager();
    	ProcesoFuncional pf = new ProcesoFuncional(idPF);
    	Query q = em.createNamedQuery("SubProceso.findSPByIndiceActividadMayor")
    			.setParameter("idPF", pf)
    ...
    public SubProceso findLastSP() {
        EntityManager em = getEntityManager();
        Query q = em.createNativeQuery("select * from subproceso order by idsubProceso desc limit 1", SubProceso.class);
        if (q.getResultList().isEmpty()) {
            return null;
        }
        return (SubProceso) q.getSingleResult();
    }
